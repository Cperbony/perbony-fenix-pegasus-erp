...
import 'package:perboni_erp_fenix/src/model/pessoa_juridica.dart';
...
class PessoaJuridicaFieldsPersiste extends PessoaJuridicaFieldsLista {
  ...
  final GlobalKey<FormState> _formKeyPessoaJuridicaPersistePage =
      GlobalKey<FormState>();
...
class PessoaJuridicaFieldsLista extends PessoaJuridicaFields {
  final GlobalKey<ScaffoldState> scaffoldKeyPessoaJuridicaListaPage =
      GlobalKey<ScaffoldState>();
}

/// Classe que declara e disponibiliza os FieldWidgets
class PessoaJuridicaFields {
  FieldWidgets<PessoaJuridica> _fieldId,
      _fieldIdPessoa,
      _fieldCnpj,
      _fieldNomeFantasia,
  ...
  set fieldIdPessoa(FieldIdPessoa fieldIdPessoa) =>
      _fieldIdPessoa = fieldIdPessoa;
  ...
  set fieldNomeFantasia(FieldNomeFantasia fieldNomeFantasia) =>
      _fieldNomeFantasia = fieldNomeFantasia;

  FieldInscricaoEstadual get fieldInscricaoEstadual => _fieldInscricaoEstadual;
  set fieldInscricaoEstadual(FieldInscricaoEstadual fieldInscricaoEstadual) =>
      _fieldInscricaoEstadual = fieldInscricaoEstadual;

  FieldInscricaoMunicipal get fieldInscricaoMunicipal =>
      _fieldInscricaoMunicipal;
  set fieldInscricaoMunicipal(
          FieldInscricaoMunicipal fieldInscricaoMunicipal) =>
      _fieldInscricaoMunicipal = fieldInscricaoMunicipal;

  FieldDataConstituicao get fieldDataConstituicao => _fieldDataConstituicao;
  set fieldDataConstituicao(FieldDataConstituicao fieldDataConstituicao) =>
      _fieldDataConstituicao = fieldDataConstituicao;

  FieldTipoRegime get fieldTipoRegime => _fieldTipoRegime;
  set fieldTipoRegime(FieldTipoRegime fieldTipoRegime) =>
      _fieldTipoRegime = fieldTipoRegime;
  ...
  void init([Object pessoaJuridica]) {
    if (pessoaJuridica == null) {
      _pessoaJuridica = PessoaJuridica("1", "1", "00000000000191",
          "Empresa de Teste", "123", "456", "10101990", "0", "0");
    } else {
...
class FieldId extends FieldWidgets<PessoaJuridica> {
  FieldId([PessoaJuridica pessoaJuridica])
      : super(object: pessoaJuridica, label: 'Id', value: pessoaJuridica?.id);

  void onSaved(v) => object?.id = value = v;

  @override
  CircleAvatar get circleAvatar => CircleAvatar(child: Text(value?.toString()));
}
